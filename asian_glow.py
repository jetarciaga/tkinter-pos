...
ITEMS_UNIT_PRICE = json.loads(
    (ASSETS / "data" / "unit_price.json").read_text(encoding="utf-8")
)
...
class ProfileWidget:
    ...
    def _load_image(self, filename):
        img_dir = ASSETS / "img" / filename
        img = Image.open(img_dir).resize((200, 200))
        self.profile_img = ImageTk.PhotoImage(img)

    def generate_widget(self):
        self._load_image("Profile.jpg")
        self.profile = tk.Label(self.frame, image=self.profile_img, text="KuyJets", compound=tk.BOTTOM)
...
class ProductWidget:
    master: tk.Tk
    orders_details_widget: "OrderDetailsWidget"
    ...
    def generate_widget(self):
        ...
        def add_orders(name, price):
            ...
            self.orders_details_widget.create_table(ORDERS)
            # Update Total Amount
            self.orders_details_widget.update_total_amount(ORDERS)
        ...
        asian_glow_set = ImageButton(**button_config)
        asian_glow_set.grid_layout(2, 1)
...
class OrderDetailsWidget:
    ...
    def modify_transaction_window(self):
        ...
        def subtract_item(orders):
            ...
            if len(selection) == 0:
                ...
                if orders[key]["quantity"] == 0:
                    messagebox.showerror("Error", "If quantity is 0 the product will be deleted.")
                else:
                    unit_price = ITEMS_UNIT_PRICE[key]
                    orders[key]["quantity"] -= 1
                    orders[key]["price"] = unit_price * orders[key]["quantity"]
        ...
        subtract_button.grid(row=1, column=0, padx=3)
        ...
        confirm_button.grid(row=1, column=2, padx=3)
        refresh_window()

    def generate_widget(self):
        title = tk.Label(self.frame, text="Transaction Details")
        title.grid(row=0, column=0, columnspan=3, sticky="ew", padx=5, pady=5)
    ...
    def create_table(self, products, receipt=False, frame=None, headers=True):
        start = 1
        if frame is None:
            frame = self.frame
        if not headers:
        ...
        table_frame = tk.Frame(frame, bd=1)
        ...
        if headers:
            ...
            for i, header in enumerate(headers):
                label = tk.Label(table_frame, text=header, relief=tk.RAISED, width=17)
                label.grid(row=0, column=i)

        # Populate products
        for i, product in enumerate(products.values(), start=start):
            label_product = tk.Label(scrollable_frame, text=product["name"], bg="white", width=17)
            label_product.grid(row=i, column=0)

            label_quantity = tk.Label(scrollable_frame, text=product["quantity"], bg="white", width=17)
            label_quantity.grid(row=i, column=1)

            label_price = tk.Label(scrollable_frame, text=product["price"], bg="white", width=17)
...
if __name__ == "__main__":
    ...
    ProfileWidget(root)
    orders_details_widget = OrderDetailsWidget(root)
    ProductWidget(root, orders_details_widget)
    root.mainloop()